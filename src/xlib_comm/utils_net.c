...
int start_connection(const char *pServerIP, unsigned short port)
{
    ...
    printf("start tcp connecting %s:%d", pServerIP, port);
    ...
    if(skt > 0)
    {
        ...
        ret = connect(skt, (struct sockaddr *)&addr, sizeof(struct sockaddr));
    }

    printf("connect %s:%d fd = %d ret = %d",pServerIP, port, skt ,ret);
...
static int __send(int sockfd, unsigned char *pBuffer, unsigned int size)
{
    int retv = 0;
_send_loop:
...
static int __recv(int sockfd, unsigned char *pBuffer, unsigned int size)
{
    int retv = 0;
_recv_loop:
