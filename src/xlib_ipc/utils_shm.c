...
s32 tbsb_write_one_packet(shm_t *pbsb, u8 *header, u8 *packet_addr, s32 packet_size)
{
    ...
    u8 *_header_addr = (u8 *)pbsb + pbsb->_header_offset;
    t_packet_info *_packet_addr = (t_packet_info *)((u8 *)pbsb + pbsb->_packet_offset);
    ...
    // if need copy header
    if (header) {
        cur_header_write_addr = _header_addr + cur_write_index * sizeof(t_header_info);
        //copy header to pbsb
        memcpy(cur_header_write_addr, header, sizeof(t_header_info));
    }
...
s32 tbsb_read_one_packet(shm_t *pbsb, u8 *header, u8 *packet_addr, s32 packet_size)
{
    ...
    // if need copy header
    if (header) {
        cur_header_read_addr = _header_addr + cur_read_index * sizeof(t_header_info);
        memcpy(header, cur_header_read_addr, sizeof(t_header_info));
    }
